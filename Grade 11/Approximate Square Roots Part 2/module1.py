...
def calc(guess: float, tolerance: float = 1e-11, num: float = num):
...
if isinstance(determine(), bool) and determine():
  im = ", which is an rational number"
  appro = ""
elif not determine():
  im = ", which is an irrational number"
  appro = "approximately "
elif determine() == 100:
  im = ", which is a complex number"
  appro = "approximately "

print(f"\n\nYou inputted {string_num}.\nIt's square root is {appro}{guess1}{im}.")