...
class save_images(keras.callbacks.Callback):
    ...
    # Creating a custom method when an epoch ends
    def on_epoch_end(self, epoch, logs=None):
        ...
        image_array = np.full((
            self.margin + (self.num_rows * (96 + self.margin)),
            self.margin + (self.num_cols * (96 + self.margin)), 3),
            255, dtype=np.uint8)
        ...
        for row in range(self.num_rows):
            for col in range(self.num_cols):
                r = row * (96 + 16) + self.margin
                c = col * (96 + 16) + self.margin
                image_array[r:r + 96, c:c + 96] = generated_images[image_count] * 255
                image_count += 1
...
class checkpoint_callback(keras.callbacks.Callback):
    ...
    def __init__(self,**kwargs):
        super(keras.callbacks.Callback, self).__init__(**kwargs)
    def on_epoch_end(self, epoch, logs=None):
        self.model.generator.save("Weights/test/generator-test")
        self.model.discriminator.save("Weights/test/discriminator-test")
        self.model.generator.save("Weights/test/generator.h5")
        self.model.discriminator.save("Weights/test/discriminator.h5")
